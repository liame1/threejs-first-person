...
const trainData = [
  ...
  let countdownTimer = 10;       // Seconds counter (resets to 10)
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize the texture update system
    ...
    // Wait for A-Frame scene to load before starting texture updates
    ...
    // Add event listener for board button if it exists
    const boardBtn = document.getElementById('boardBtn');
    if (boardBtn) {
      boardBtn.addEventListener('click', function() {
        playerIsBoarded = !playerIsBoarded;
        console.log("Player boarding status toggled to: " + playerIsBoarded);
      });
    }
  });